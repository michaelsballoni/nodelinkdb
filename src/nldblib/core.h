...
namespace nldb
{
    ...
    {
        if (!isWord(name))
            throw nldberr("Invalid table name: " + toNarrowStr(name));
    }

    inline void validateColumnName(const std::wstring& name)
    {
        if (!isWord(name))
            throw nldberr("Invalid column name: " + toNarrowStr(name));
    }

    inline void validateOperator(const std::wstring& op)
    {
        if (!isValidOp(op))
            throw nldberr("Invalid query operator: " + toNarrowStr(op));
    }

    inline void validateParameterName(const std::wstring& name)
    {
        if (name.empty() || name[0] != '@' || !isWord(name.substr(1)))
            throw nldberr("Invalid parameter name: " + toNarrowStr(name));
    }
