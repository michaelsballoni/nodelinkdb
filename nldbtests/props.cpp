...
namespace nldb
{
	...
	public:
		TEST_METHOD(TestProps)
		{
			...
			props::set(db, item_type_id, node.id, strings::get_id(db, L"foo"), strings::get_id(db, L"bar"));
			prop_str = props::summarize(db, props::get(db, item_type_id, item_id));
			Assert::IsTrue(prop_str == L"foo bar");

			props::set(db, item_type_id, node.id, strings::get_id(db, L"blet"), strings::get_id(db, L"monkey"));
			prop_str = props::summarize(db, props::get(db, item_type_id, item_id));
			Assert::IsTrue(prop_str == L"blet monkey\nfoo bar");

			props::set(db, item_type_id, node.id, strings::get_id(db, L"foo"), -1);
			prop_str = props::summarize(db, props::get(db, item_type_id, item_id));
			Assert::IsTrue(prop_str == L"blet monkey");

			props::set(db, item_type_id, node.id, -1, -1);
			prop_str = props::summarize(db, props::get(db, item_type_id, item_id));
			Assert::IsTrue(prop_str.empty());
		}
	};
